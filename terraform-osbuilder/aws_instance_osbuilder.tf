...
resource "aws_instance" "osbuilder" {
  ...
  provisioner "local-exec" {
    ...
    command     = "ansible all -i '${self.public_ip},' -u ec2-user --private-key ssh_key/osbuilder_priv_key -m wait_for_connection"
  }

  provisioner "local-exec" {
    environment = { ANSIBLE_HOST_KEY_CHECKING = "False" }
    command     = "ansible-playbook -i '${self.public_ip},' -u ec2-user --private-key ssh_key/osbuilder_priv_key ansible/oraclelinux/osbuilder.yml"
  }
